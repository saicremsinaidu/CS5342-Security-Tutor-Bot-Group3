...
def make_answer(query: str, k: int = 4) -> Tuple[str, List[Dict[str, str]]]:
    ...
    # --- FIX: remove duplicate chunks before joining ---
    seen = set()
    unique_chunks = []
    for d, _ in hits[:k]:
        if d and d.strip() not in seen:
            unique_chunks.append(d.strip())
            seen.add(d.strip())
    ctx = " ".join(unique_chunks)
    ...
    # --- Deduplicate similar sentences after selection ---
    final_sents, seen_lines = [], set()
    for s in chosen:
        low = s.lower()
        if low not in seen_lines:
    ...
    for _, m in hits[:k]:
        ...
        if src not in sources:
            sources.append(src)
    cite_str = " ".join(f"[{i+1}: {s}]" for i, s in enumerate(sources, start=1))
    answer_text = " ".join(final_sents) or "I found related material but could not extract a concise answer."
...
def generate_quiz(topic: str = "", n: int = 5) -> Dict[str, Any]:
    ...
    rr = itertools.cycle(range(len(per_source)))
    items, seen = [], set()
    ...
    while len(items) < n and len(seen) < 500:
        ...
        if tmod == 0:
            ...
            items.append({"type": "open", "q": f"Briefly explain: {s}", "answer": s, "sources": [source]})
